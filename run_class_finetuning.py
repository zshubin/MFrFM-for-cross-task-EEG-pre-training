...
#########
    ...
    parser.add_argument('--drop', type=float, default=0.5, metavar='PCT', #0.5
                        help='Dropout rate (default: 0.)')
    parser.add_argument('--attn_drop_rate', type=float, default=0.3, metavar='PCT', #0.3
                        help='Attention dropout rate (default: 0.)')
    parser.add_argument('--drop_path', type=float, default=0.3, metavar='PCT',  #0.3
...
def main(args):
    ...
    if args.layer_decay < 1.0:
        ...
        assigner = LayerDecayValueAssigner([0.]*num_layers+[1.])#14-(0..13)
    else:
        assigner = None
...
if __name__ == '__main__':
    opts = get_args()
    experiment_name = 'amplitude_scale_frft_mae'
    target_task_list = ['MI', 'SSVEP']
    for target_task in target_task_list:
        ...
        for source_task in target_task_list:
            for session in session_list:
                # sub_list = os.listdir(os.path.join('/home/data/zhangshubin/processed_openbmi_{}_data'.format(target_task), session))
                sub_list = ['s1', 's16', 's20', 's45', 's47']
                print(len(sub_list))
                record_file = open(
                    './record_prob_{}_{}_to_{}_{}.txt'.format(experiment_name, source_task.lower(), target_task.lower(), session),
                    'w')
                for subj_index in range(len(sub_list)):
                    subj_number = sub_list[subj_index]
                    print(subj_number)
                    opts.data_path = os.path.join(
                        '/home/data/zhangshubin/processed_openbmi_{}_data/'.format(target_task), session, subj_number)
                    opts.finetune = os.path.join(
                        '/home/data/zhangshubin/EEG_pretraining_model/model_{}'.format(experiment_name), source_task.lower(),
                        session, subj_number, 'model.pth')
                    opts.output_dir = './finetune_frft_mae_model'
                    opts.log_dir = None
                    if opts.output_dir:
