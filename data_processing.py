...
# "split openbmi data"
# sess_list = os.listdir('./openbmi')
# task_list = ['ERP', 'MI', 'SSVEP']
# for task in task_list:
#     os.makedirs(os.path.join('openbmi', task))
#     for sess in sess_list:
#         if sess == 'session1':
#             s = 'sess01'
#         else:
#             s = 'sess02'
#         os.makedirs(os.path.join('openbmi', task, sess))
#         sub_list = os.listdir(os.path.join('openbmi', sess))
#         for sub in sub_list:
#             os.makedirs(os.path.join('openbmi', task, sess, sub))
#             shutil.move(os.path.join('openbmi', sess, sub, '{}_subj{}_EEG_{}.mat'.format(s, sub.zfill(2), task)),
#                         os.path.join('openbmi', task, sess, sub, '{}_subj{}_EEG_{}.mat'.format(s, sub.zfill(2), task)))



"process public dataset"
phase_list = ['train', 'test']
data_path = '/home/data/zhangshubin/OpenBMI/MI'
processed_data_path = '/home/data/zhangshubin/processed_openbmi_MI_data/'
        ...
        for phase in phase_list:
            ...
            ori_data = \
                sio.loadmat(os.path.join(data_path, sess, sub,
                                         "sess{}_subj{}_EEG_MI.mat".format(str(sess_index + 1).zfill(2),
                                                                              sub[1:].zfill(2))),
                            variable_names=["EEG_MI_{}".format(phase)])['EEG_MI_{}'.format(phase)]
            data = ori_data['x'][0][0]
...
# "process 40-target data"
# phase_list = ['train', 'test']
# data_path = '/home/data/zhangshubin/40_target'
# processed_data_path = './processed_40target_data/'
# os.makedirs(processed_data_path, exist_ok=True)
# sub_list = os.listdir(data_path)
# label_list = list(range(0,40))
# for sub_index, sub in enumerate(sub_list):
#     print(sub[:-4])
#     os.mkdir(os.path.join(processed_data_path, sub[:-4]))
#     ori_data = sio.loadmat(os.path.join(data_path, sub_list[sub_index]))['data']
#     os.makedirs(os.path.join(processed_data_path, sub[:-4], 'train'), exist_ok=True)
#     os.makedirs(os.path.join(processed_data_path, sub[:-4], 'test'), exist_ok=True)
#     for label in label_list:
#         os.mkdir(os.path.join(processed_data_path, sub[:-4], 'train', str(label)))
#         os.mkdir(os.path.join(processed_data_path, sub[:-4], 'test', str(label)))
#         test_trail = random.randrange(0, 6)
#         trail_list = list(range(0, 6))
#         trail_list.remove(test_trail)
#         for i in trail_list:
#             trail_bci_data = ori_data[:,:,label,i]
#             np.save(os.path.join(processed_data_path, sub[:-4], 'train', str(label),
#                                  '{}_{}.npy'.format(sub[:-4], str(i))),trail_bci_data)
#         trail_bci_data = ori_data[:, :, label, test_trail]
#         np.save(os.path.join(processed_data_path, sub[:-4], 'test', str(label),
#                              '{}_{}.npy'.format(sub[:-4], str(test_trail))), trail_bci_data)
